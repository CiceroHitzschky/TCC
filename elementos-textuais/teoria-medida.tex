...
Um par ordenado $(X, \Sigma)$  constituído de um conjunto $X$ e uma \sigal sobre $X$ é chamado de  \textbf{espaço mensurável} . \index{espaço mensurável}
...
% \begin{prova}
%     Demonstraremos  por indução sobre $n$. 
%     Seja $X = \left\{n \in \N;\ \displaystyle \bigcap_{j = 1}^n A_j \in \Sigma\right\}$. 
%     Observe que $2 \in X$, pois se $A_1$ e $A_2$ são elementos de $\Sigma$, então $A_1 \cap A_2 \subset A_1 \in \Sigma$, ou seja, $A_1 \cap A_2 \in \Sigma$.
%     Suponha que $k \in X$ para algum $k \in \N$.
%     Vamos mostrar que o sucessor de $k$ também está em $X$.
%     Por definição, 
%     \begin{equation}
%         \displaystyle \bigcap_{j = 1}^{k+1} A_j = 
%         \left(\displaystyle \bigcap_{j = 1}^k A_j\right) \cap A_{k+1}   
%     \end{equation}

%     Como $k \in X$, temos que $\displaystyle \bigcap_{j = 1}^k A_j \in \Sigma$.
%     Além disso, $A_j \in \Sigma$ para qualquer $1 \leq j \leq k+1$, isto é, $A_{k+1} \in \Sigma$.
%     Com isso, vemos que $\displaystyle \bigcap_{j = 1}^{k+1} A_j$ é a intersecção de dois conjuntos que estão em $\Sigma$.
%     Logo, pela base de indução, tem que ser um elemento de $\Sigma$.
%     Com isso, concluímos que $2 \in X$, $k \in X \Rightarrow k+1 \in X$.
%     Portanto, $X = \N$ como desejávamos.
% \end{prova}
...
Esta \sigal $\Sigma$   definimos como a \textit{menor} \sigal gerada por $A$. \index{gerada por um conjunto}
...
\label{prop:interseção-sigmas}
    Seja $(\Sigma_j)$ uma sequência finita de $\sigma$-álgebras de um conjunto $X$, então 
    $\Sigma = \displaystyle \bigcap_{j =1}^n\Sigma_j$, com $n \geq 2$, também é uma \sigal do conjunto $X$.
    
\end{proposition}
\begin{prova}
    Provaremos por indução sobre $n$.
    Se $n = 2$, o resultado é verificado imediatamente pela proposição \ref{prop:interseção-2sigmas}.
    Suponha que isso se verifique para algum $k \in \N$, isto é, $\displaystyle \bigcap_{j =1}^k\Sigma_j$ é uma \sigal de $X$.
    Vamos checar para o sucessor de $k$.
    Ora, pela associatividade da interseção, temos 
    \begin{align*}
        \displaystyle \bigcap_{j =1}^{k+1}\Sigma_j = \left(\displaystyle \bigcap_{j =1}^k\Sigma_j\right) \cap A_{k+1}
  \end{align*}
Denote $\displaystyle \bigcap_{j =1}^k\Sigma_j = H$. 
Sabemos por hipótese de indução que $H$  e $A_{k+1}$ são  \sigals de $X$. 
Segue, pela base de indução, que $H \cap A_{k+1}$ é uma \sigal de $X$.
Portanto, $\displaystyle \bigcap_{j =1}^{k+1}\Sigma_j$ é uma \sigal de $X$ como queríamos.
\end{prova}
...
Perceba que até aqui trabalhamos o conceito de \sigal de maneira abstrata sendo utilizada em um conjunto qualquer. 
Agora, estudaremos um a Álgebra de Borel.
Esta \sigal é uma \sigal do conjunto dos números reais que é definida da seguinte forma
...
Assim, $(-\infty, x)$ não é a única forma dos elementos de $\borel$. 
...
Em particular, poderíamos ter definido a $\borel$ por meio da \sigal gerada por intervalos do tipo $(a,b)$ com $a,b \in \R$. Antes de provamos este fato, observe que podemos decompor intervalos reais como a união de intervalos. 
